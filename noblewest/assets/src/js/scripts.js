...
(function ($) {
	...
	$(document).ready(function() {
		...
		var optionalInput = '';
		// Handle the Click to call to action quiz    
		const handleCTAQuiz = ()=>{
				$('.quiz .quiz-launch').on('click',(e)=>{
					e.preventDefault();
		...
		const handleClickToAnswer = ()=>{
			$('.happiness-form .question').on('click',(e)=>{
				if(!e.currentTarget.getAttribute('disabled')){
					...
					$(`.happiness-form .step-${currentQuizStep} .question`).attr('disabled',true);
					
					setTimeout(()=>{
						$(`.happiness-form .quiz-content.step-${currentQuizStep}`).addClass( "slide-out" );
						if((currentQuizStep + 1) === 6){
							const getValue = e.currentTarget.dataset.value;
							...
							const getValue = parseInt(e.currentTarget.dataset.value);               
							fullQuizValue = fullQuizValue + getValue;
							$(`.happiness-form .quiz-content.step-${currentQuizStep}`).addClass( "slide-out" );
							$(`.happiness-form .quiz-content.step-${currentQuizStep}`).addClass( "d-none" );
								$(`.happiness-form .quiz-content.step-${currentQuizStep + 1}`).removeClass( "d-none" );
								$(`.happiness-form .quiz-content.step-${currentQuizStep + 1}`).addClass( "slide-in" );
							$(`.happiness-form .progress-steps .steps .step[data-step="${currentQuizStep + 1}"]`).addClass('active');
